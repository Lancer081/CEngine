...
// UCI "startime" command time holder
int startime = 0;
...
static inline int negamax(int depth, int alpha, int beta)
{
	...
	for (int i = 0; i < moves->count; i++)
	{
		...
		else
		{
			...
			if (moves_searched >= full_depth_moves && depth >= reduction_limit && !in_check && !get_move_capture(moves->moves[i]) && !get_move_promoted(moves->moves[i]))
	...
	for (int current_depth = 1; current_depth <= depth; current_depth++)
	{
		...
		if (pv_length[0])
		{
			if (score > -mate_value && score < -mate_score)
				printf("info score mate %d depth %d nodes %ld time %d pv ", -(score + mate_score), current_depth, nodes, get_time_ms() - startime);
			else if (score > mate_score && score < mate_value)
				printf("info score mate %d depth %d nodes %ld time %d pv ", (mate_score - score), current_depth, nodes, get_time_ms() - startime);
			else
				printf("info score cp %d depth %d nodes %ld time %d pv ", score, current_depth, nodes, get_time_ms() - startime);
...
void parse_go(char* command)
{
	...
	printf("time:%d start:%d stop:%d depth:%d timeset:%d\n",
		time, startime, stoptime, depth, timeset);
